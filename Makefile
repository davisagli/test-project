...
build-acceptance-servers: ## Build Acceptance Servers
	...
	@docker build backend -t collective/test-project-backend:acceptance -f backend/Dockerfile.acceptance
	@echo "Build acceptance frontend"
	@docker build frontend -t collective/test-project-frontend:acceptance -f frontend/Dockerfile

.PHONY: start-acceptance-servers
start-acceptance-servers: build-acceptance-servers ## Start Acceptance Servers
	@echo "Start acceptance backend"
	@docker run --rm -p 55001:55001 --name test-project-backend-acceptance -d collective/test-project-backend:acceptance
	@echo "Start acceptance frontend"
	@docker run --rm -p 3000:3000 --name test-project-frontend-acceptance --link test-project-backend-acceptance:backend -e RAZZLE_API_PATH=http://localhost:55001/plone -e RAZZLE_INTERNAL_API_PATH=http://backend:55001/plone -d collective/test-project-frontend:acceptance

.PHONY: stop-acceptance-servers
stop-acceptance-servers: ## Stop Acceptance Servers
	@echo "Stop acceptance containers"
	@docker stop test-project-frontend-acceptance
	@docker stop test-project-backend-acceptance
